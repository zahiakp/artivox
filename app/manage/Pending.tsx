    ...
    const fetchPrograms = useCallback(async () => {
        ...
        try {
            let response;
                ...
                response = await getProgramsbyStatusandPagination("pending", params.toString(), proEndpoint);
        ...
        return (
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                    ...
                    <div className="p-[6px] hidden md:flex px-4 rounded-lg items-center border border-gray-200 gap-3 w-1/3">
                        <input
                            id="search-input"
                            value={inputValue}
                            onChange={handleSearchInputChange}
                            ...
                            {ongoing.map((program: any) => (
                                <tr key={program.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                       {program.participants > 0 ? <div data-tip="Report"
                              className="tooltip text-white p-2 rounded-lg w-fit bg-gradient-to-tr from-green-400 to-emerald-600 cursor-pointer mr-5"
                              onClick={() =>
                                setConfirm({ program, type: "reporting" })
                              }
                            >
                              <IconMenuElements/>
                            </div>: <p className="text-red-600">No participants</p>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>

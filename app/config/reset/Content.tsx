...
function Content() {
    ...
    const proEndpoint = useMemo(() => {
        ...
        const partEndpoint = role =='admin'?"participants":"campusparticipants";
    ...
    const fetchPrograms = useCallback(async () => {
        ...
        try {
          let response;
            response = await getProgramsbyStatusandPagination("reporting,ongoing,finished,resulted,judged,announced", params.toString(), proEndpoint);
    ...
    const renderContent = () => {
        if (loading) {
            return (
                <div className='bg-violet-50 flex items-center w-full h-60 rounded-lg border border-violet-200'>
                    <LDRloader />
                </div>
            );
        }
        ...
        return (
          <>
            <p className="text-xl  mb-5 font-semibold">Reset Programs</p>
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h3 className="text-lg font-medium text-gray-900">
                  <span className="font-bold">{totalRecords}</span> Program(s)
                </h3>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="cursor-pointer px-6 py-3 text-left text-xs font-medium text-gray-500 hidden md:block uppercase tracking-wider">
                        Category
                      </th>
                      <th className="cursor-pointer px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Status
                      ...
                      <tr key={program.id}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <img
                              src={`https://placehold.co/40x40/f7e6ff/7d00b3?text=${
                                getFirstLastInitials(program.name) || "AV"
                              }`}
                              alt={program.name}
                            ...
                            <div className="ml-4">
                              ...
                              <div className="text-[12px] text-gray-500 md:hidden">
                                {program.category}
                              </div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap uppercase text-sm hidden md:block text-gray-500">
                          {program.category}
                        </td>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setReport({
                                    program,
                                    participants:
                                      program?.participantsData,
                                    type: "assign",
                                  })
                                }
                              >
                                <MdOutlineResetTv className="text-xl" />
                                {/* assign code  */}
                              </button>
              ...
              {report && (
                <ResetModal close={setReport} data={report} fetch={fetchPrograms} />
              )}
            </div>
          </>
        );

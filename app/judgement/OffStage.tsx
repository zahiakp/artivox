    ...
    const fetchPrograms = useCallback(async () => {
        ...
        try {
            let response;
                ...
                response = await getProgramsforJudgment("finished", params.toString(), proEndpoint, 0);
            

            if (response && response.data) {
                const filteredData = response.data.filter((item: any) => item.isStage === 0);
                            ...
                            {ongoing.map((program: any) => (
                                <tr key={program.id}>
                                    ...
                                    <td className="px-6 py-4 flex whitespace-nowrap text-sm font-medium space-x-2">
                                        ...
                                        {(role == 'admin') && program.participantsData.some((item: any) => item.status === "finished" && item.mark !== 0) && program.status === 'finished' && (
                                            <button
